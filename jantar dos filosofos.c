...
void* rotina_filosofo(void* arg) {
    ...
    while (1) {
        ...
        pthread_mutex_lock(&hashi[hashi_esquerdo]); 
        pthread_mutex_lock(&hashi[hashi_direito]);

        printf("Filósofo %d está comendo!\n", id_filosofo + 1);
        sleep(1);

        //soltar hashis
        pthread_mutex_unlock(&hashi[hashi_direito]);
        pthread_mutex_unlock(&hashi[hashi_esquerdo]);
